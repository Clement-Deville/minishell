...
/*   Updated: 2024/06/25 10:24:57 by cdeville         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <minishell.h>

int	do_child(char **command, t_node *node, t_dblist **env)
{
...
int	do_parent(char **command, t_node *node, t_dblist **env, int pid)
{
	int	status;
...
int	exec_builtin(t_node *node, t_dblist **env)
{
	...
	if (pid == 0)
	{
		...
		if (red_status)
			exit (clean_and_exit(1));
		do_child(command, node, env);
	}
	return (do_parent(command, node, env, pid));
}
