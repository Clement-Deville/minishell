...
int	exec_simple_cmd(t_node *node, t_mini_env *ms, t_bool piped)
{
	...
	if (ft_is_builtin((node->c_cmd->expand)[0])
		&& (!ft_strncmp((node->c_cmd->expand)[0], "cd", ft_strlen("cd"))
			// || !ft_strncmp((node->c_cmd->expand)[0], "pwd", ft_strlen("pwd"))
			|| !ft_strncmp((node->c_cmd->expand)[0], "export", ft_strlen("export"))
			|| !ft_strncmp((node->c_cmd->expand)[0], "exit", ft_strlen("exit"))))
	{
		return (ft_exec_builtin(node->c_cmd->expand, ms));
	}
	else if (ft_is_builtin((node->c_cmd->expand)[0]))
	{
		...
		else
		{
			...
			{
				status = ft_exec_builtin(node->c_cmd->expand, ms);
				ft_clean_ms();
				exit(status);
			}
			waitpid(fork_pid, &status, 0);
...
void	start_exec(t_node *node, t_mini_env *ms)
{
	if (!node)
		...
		get_ms()->exit = exec_node(node, ms, FALSE, 0);
}
